...
# Reformulado como dos restricciones lineales con OR-Tools:
#   Σ inicio >= Σ asig - 1
#   Σ inicio >= 0  (implícito, son BoolVar)
# El optimizador minimiza inicios (porque cada inicio aporta a la función objetivo
# vía el costo de setup en R1a), entonces elegirá el menor valor factible que es
# exactamente max(0, N-1).
...
for d_idx, d in enumerate(m.dias):
    for l in m.lineas:
        ...
        # Acotar superior para que inicio nunca sea 1 cuando asig=0
        # (esto es importante para mantener la integridad de la variable)
        for k in m.skus:
            if l in m.pares_sku_linea[k]:
                m.model.Add(m.inicio[(d, k, l)] <= m.asig[(d, k, l)])
```

### Paso 3 — Verificar que R1a sigue siendo coherente
...
### Paso 4 — Eliminar el comentario de R4 sobre día 0
...
| quiebre | 0 | 0 |
| ofts_con_paga_setup | 130 | **<60** (R12 elimina los "primer SKU del día") |
| objective_value | 521 mil M | <300 mil M |

El cambio crítico es **`ofts_con_paga_setup`**: con R12 debería bajar drásticamente. Si baja menos del 30%, hay algo mal en la implementación de R12 — re-verificar.
...
### Riesgo 1 — Comportamiento del solver con R12
...
**Mitigación**: si pasa, considerar reintroducir un peso pequeño en función objetivo que penalice la fragmentación entre días distintos (no setups intra-día). O esperar a F2 (sequencer) que va a optimizar dentro del día con la matriz real.

### Riesgo 2 — Coherencia con OFs aprobadas
...
### Riesgo 3 — La "decisión del Gerente" implica cambio de proceso operativo
...
| Archivo | Cambio |
|---|---|
| `forecast/optimizer.py` | Reemplazar bloque de detección de `inicio` por la restricción agregada R12. |
