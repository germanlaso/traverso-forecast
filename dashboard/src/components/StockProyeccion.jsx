...
// ── Buscador de SKU con dropdown filtrable ───────────────────────────────────────
// Portado del patrón SkuSearch de App.js (pestaña Forecast). Autocontenido: usa el
// C y TIPO_COLOR de este módulo (no depende del scope de App.js). Difiere del original
// en que la columna lateral muestra el badge de tipo (PROD/IMP/MAQ) en vez de volumen,
// dato que no existe en la lista de /plan/params usada por esta pestaña.
// Deuda de refactor: extraer a components/SkuSearch.jsx compartido con App.js.
    ...
    <div ref={ref} style={{ position: "relative", flex: 1, maxWidth: 480 }}>
      <input
        style={{ fontSize: 13, padding: "7px 10px", borderRadius: 6,
          border: `0.5px solid ${focused ? C.teal : C.border}`, background: "#fff",
      ...
      {open && filtered.length > 0 && (
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 200,
          ...
          {query.trim() === "" && (
            <div style={{ fontSize: 10, color: C.textMuted, padding: "6px 12px",
              borderBottom: `0.5px solid ${C.border}` }}>
              Primeros 100 · escribe para buscar en todos
            </div>
          )}
          {filtered.map((sk, i) => {
            const tc = TIPO_COLOR[sk.tipo];
            return (
              <div key={sk.sku} onMouseDown={() => select(sk)}
                ...
                {sk.tipo && (
                  <span style={{ marginLeft: "auto", flexShrink: 0, fontSize: 9, fontWeight: 700,
                    padding: "1px 6px", borderRadius: 3,
                    background: tc ? tc.bg : C.grayLt, color: tc ? tc.color : C.textMuted }}>
                    {sk.tipo === "PRODUCCION" ? "PROD" : sk.tipo === "IMPORTACION" ? "IMP" : sk.tipo === "MAQUILA" ? "MAQ" : sk.tipo}
                  </span>
                )}
              </div>
            );
    ...
    <div style={s.wrap}>
      ...
      <div style={s.card}>
        <div style={s.row}>
          ...
          <SkuSearch
            skus={skuList.length
              ? skuList.map((sk) => ({ sku: sk.sku, descripcion: sk.desc ?? params[sk.sku]?.desc, tipo: sk.tipo }))
              : Object.entries(params).map(([k, v]) => ({ sku: k, descripcion: v.desc, tipo: v.tipo }))}
            value={selSku}
            onChange={setSelSku}
          />
